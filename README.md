...
**说明：**
- `python3-bpfcc` (Ubuntu) / `python3-bcc` (RHEL/openEuler): 必须，Python `bcc` 模块（代码中 `from bcc import BPF`）
- `linux-headers` (Ubuntu) / `kernel-headers` (RHEL/openEuler): 通常需要，编译 eBPF 程序必需（如果报错再安装）
...
# 安装依赖（openEuler）
sudo dnf install -y python3-bcc kernel-headers
