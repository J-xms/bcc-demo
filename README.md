...
```bash
sudo python3 function_tracer.py \
  -r rules/tcp_trace_config.json \
  -o results/ \
  -t 10 \
  -f 10
```
...
```bash
sudo python3 function_tracer.py \
  -r rules/usdt_trace_config.json \
  -o results/ \
  -t 5
```
...
```bash
sudo python3 function_tracer.py \
  -r rules/mixed_trace_config.json \
  -o results/ \
  -t 5 \
  -f 100
```
...
# 安装依赖
sudo dnf install -y bcc-tools kernel-devel
